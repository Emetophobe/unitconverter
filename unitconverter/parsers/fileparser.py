...
def load_dimensions() -> dict[str, Dimension]:
    ...
    try:
        ...
        raise DefinitionError(f"Failed to load dimensions from {e.filename} ({e.strerror})")
...
def load_units() -> dict[str, Definition]:
    ...
    if not files:
        raise DefinitionError(f"Missing pre-defined unit files in {path}")
    ...
    for filename in files:
        ...
        try:
            ...
            raise DefinitionError(f"Failed to load units from {e.filename} ({e.strerror})")

        # Make sure to pop dimension to separate it from the unit data
        try:
            dimensions = data.pop("dimension")
        except KeyError:
            raise DefinitionError("Unit file is missing required dimension", filename)

        # Convert toml dictionary into a dictionary of unit definitions
        for name, args in data.items():
            if name in units:
                raise DefinitionError(f"{name} is already defined (unit names must be unique)")
